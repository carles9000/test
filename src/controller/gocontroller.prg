...
CLASS GoController
	...
	METHOD VerDatos( oController )
	METHOD Login( oController )
	METHOD Autentica( oController )
...
METHOD New( oController ) CLASS GoController
...
METHOD Go( oController ) CLASS GoController

	local cUser := oController:oRequest:Get( 'user' )
	local cPsw  := oController:oRequest:Get( 'psw' )
			
	
	IF cUser == 'pol' .and. cPsw == '12345'
		cToken := 'XEVI-POL'
		::VerDatos( cToken )
	ELSE
		::Login()
...
METHOD VerDatos( cToken  ) CLASS GoController

	  SetCookie( 'APPXEVI', cToken , 60 )
	? 'Muestro mis datos...'
...
METHOD Login( oController ) CLASS GoController

	? '<form action="' + Route( 'milogin' ) + '"  method="POST">'
...
METHOD Autentica( oController ) CLASS GoController
